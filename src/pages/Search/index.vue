        ...
        <div class="bread">
          ...
          <ul class="fl sui-tag">
            ...
            <li class="with-x" v-if="params.keyword">
              {{ params.keyword }}<i @click="removeKeywords">×</i>
            </li>
        ...
        <div class="details clearfix">
          <div class="sui-navbar">
            <div class="navbar-inner filter">
              <ul class="sui-nav">
                <li :class="{ active: isShow }" @click="changeOrder('1')">
                  <a
                    >综合
                    <span v-show="isShow"
                ...
                <li :class="{ active: !isShow }" @click="changeOrder('2')">
                  <a
                    >价格
                    <span v-show="!isShow"
                      ><span v-show="!isDesc">↑</span
          ...
          <Pagination :pageNo="params.pageNo" :pageSize="params.pageSize" :total='total' :continues='5' @getPageNo="getPageNo"/>
        </div>
      </div>
    </div>
  </div>
...
export default {
  ...
  methods: {
    getData() {
      this.$store.dispatch("search/postSearchList", this.params);
    },
    removeCategoryName() {
      this.params.categoryName = undefined; //参数不会传给后台
      this.params.category1Id = ""; //传空字符串给后台
    ...
    changeOrder(flag) {
      let orderFlag = this.params.order.split(":")[0];
      let orderSort = this.params.order.split(":")[1];
    ...
    async addShopCart(goodsId) {
      try {
        await this.$store.dispatch('detail/addToCart', {goodsId:goodsId, 'goodsNum':1});
        // 路由跳转
