<template>
  <div class="pagination">
    ...
    <button v-if="startNumAndEndNum.end < maxPage-1">···</button>
    <button v-if="startNumAndEndNum.end < maxPage" @click="$emit('getPageNo', maxPage)" :class="{active:pageNo==maxPage}">{{maxPage}}</button>
...
<script>
  export default {
    ...
    computed: {
        ...
        startNumAndEndNum () {
            const { continues, pageNo, maxPage } = this;
            let start = 0, end = 0;
